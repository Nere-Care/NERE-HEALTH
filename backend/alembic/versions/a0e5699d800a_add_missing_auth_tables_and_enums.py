...
def downgrade():
    ...
    op.drop_constraint(
        "fk_chatbot_sessions_medecin_id", "chatbot_sessions", type_="foreignkey"
    )
    ...
    token_rotation_reason = postgresql.ENUM(
        "proactive", "expiration", "suspicion", "manual", name="token_rotation_reason"
    )
    token_rotation_reason.drop(op.get_bind(), checkfirst=True)

    token_revoke_reason = postgresql.ENUM(
        "logout",
        "suspicion",
        "admin_revoke",
        "password_change",
