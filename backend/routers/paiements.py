...
from ..models import Medecin, Paiement, Patient, RendezVous, User
from ..schemas import (
...
async def list_paiements(
    ...
    if current_user.role != "admin":
        stmt = stmt.where(
            ...
            | (Paiement.patient_id == current_user.id)
        )
...
@router.post(
    "/paiements", response_model=PaiementRead, status_code=status.HTTP_201_CREATED
)
...
async def create_paiement(
    ...
    if not patient_profile:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Profil patient introuvable"
        )
    ...
    if not medecin_profile:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Profil médecin introuvable"
        )

    paiement = Paiement(**paiement_create.model_dump(exclude_unset=True))
    db.add(paiement)
    try:
        db.commit()
        db.refresh(paiement)
...
async def handle_stripe_webhook(
    ...
    paiement = (
        db.query(Paiement).filter(Paiement.reference_fournisseur == session_id).first()
    )
    if paiement:
...
async def read_paiement(
    ...
    if not paiement:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Paiement non trouvé"
        )
