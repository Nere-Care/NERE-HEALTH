...
def _get_solde_details_xaf(db: Session, medecin_id: UUID):
    ...
    solde_xaf = max(total_gagne_xaf - deja_retire_xaf, Decimal(0))
    return total_gagne_xaf, deja_retire_xaf, solde_xaf


def _get_solde_disponible(db: Session, medecin_id: UUID) -> Decimal:
    _, _, solde_xaf = _get_solde_details_xaf(db, medecin_id)
    return solde_xaf
...
async def creer_retrait(
    ...
    total_gagne_xaf, deja_retire_xaf, solde_xaf = _get_solde_details_xaf(db, current_user.id)
