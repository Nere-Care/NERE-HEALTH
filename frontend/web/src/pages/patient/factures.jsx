...
import { Download, Eye, CreditCard, CheckCircle, Clock, XCircle } from 'lucide-react';
...
export default function Factures({ darkMode }) {
  ...
  const filtres = [
    ...
    { label: langue === 'fr' ? "Payé" : "Paid", val: "Payé" },
    { label: langue === 'fr' ? "En attente" : "Pending", val: "En attente" },
    { label: langue === 'fr' ? "Annulé" : "Cancelled", val: "Annulé" },
  ];
    ...
    <div className={`p-4 min-h-screen ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
      ...
      <div className="mb-6">
        <h1 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>
          {langue === 'fr' ? "Mes Factures" : "My Bills"}
        </h1>
        <p className={`text-sm mt-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
          {langue === 'fr' ? "Historique de vos paiements et factures" : "Your payment history and invoices"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: langue === 'fr' ? "Total payé" : "Total Paid", value: `${total.toLocaleString()} FCFA`, icon: CheckCircle, color: "bg-green-100 text-green-500" },
          { label: langue === 'fr' ? "En attente" : "Pending", value: `${enAttente.toLocaleString()} FCFA`, icon: Clock, color: "bg-orange-100 text-orange-500" },
          { label: langue === 'fr' ? "Factures totales" : "Total Invoices", value: factures.length, icon: CreditCard, color: "bg-blue-100 text-blue-500" },
        ].map((stat, index) => {
      ...
      <div className={`rounded-2xl shadow overflow-hidden ${darkMode ? "bg-gray-800" : "bg-white"}`}>
        <table className="w-full text-sm">
          <thead className={darkMode ? "bg-gray-700" : "bg-gray-50"}>
            <tr>
              {[
                langue === 'fr' ? "N° Facture" : "Invoice #",
                langue === 'fr' ? "Date" : "Date",
                langue === 'fr' ? "Médecin" : "Doctor",
                langue === 'fr' ? "Acte" : "Service",
                langue === 'fr' ? "Montant" : "Amount",
              ...
              <tr key={f.id} className={`border-t transition-all
                ...
                <td className="px-5 py-4">
                  <span className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full font-semibold w-fit
                    ${f.statut === "Payé" ? "bg-green-100 text-green-600"
                      : f.statut === "En attente" ? "bg-orange-100 text-orange-500"
                      : "bg-red-100 text-red-500"}`}>
                    {f.statut === "Payé" && <CheckCircle size={10} />}
                    {f.statut === "En attente" && <Clock size={10} />}
                    {f.statut === "Annulé" && <XCircle size={10} />}
                    {langue === 'fr' ? f.statut : f.statut === "Payé" ? "Paid" : f.statut === "En attente" ? "Pending" : "Cancelled"}
                  </span>
                </td>
                <td className="px-5 py-4">
